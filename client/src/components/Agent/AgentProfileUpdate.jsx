...
export default function AgentProfile() {
    ...
    useEffect(() => {
        axios.get('/agent/getagentdetails').then((response) => {
    ...
    async function registerAgent(ev) {
        ev.preventDefault();
        try {
            await userSchema.validate({ name, email, number }, { abortEarly: false });
            await axios.post('/agent/updateuser', {
                name, email, number
            });
    ...
    async function registerPassword(ev) {
        ev.preventDefault();
        try {
            await axios.post('/agent/updatepassword', {
        ...
        <div>
            <Box sx={{ width: '100%' }}>
                ...
                <CustomTabPanel value={value} index={0}>
                    <form className="max-w-md mx-auto" onSubmit={registerAgent}>
